...
def test2(model, analogy_file):
    ...
    with open(analogy_file) as f:
        for line in f:
            ...
            print(w1,w2,w3,w4)
            vector = model[w1] - model[w2] + model[w3]
...
if __name__ == "__main__":
    ...
    print(corr, pvalue)
    print(acc)
    table.append('{0:.3f} ({0:.3f})'.format(corr, pvalue))
    table.append('{.3f}'.format(acc))
    del model
    gc.collect()
    ...
    #model.wv.save_word2vec_format('w2v_brown_model.txt', binary=False)
